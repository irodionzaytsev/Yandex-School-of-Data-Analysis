...
public:
    ...
    int getIndex(int key) {
        ...
        /*
        int lower = 0;
        int upper = ordered_data_.size();
        while (upper - lower > 1) {
            int mid = (lower + upper) / 2;
            if (ordered_data_[mid] <= key) {
                lower = mid;
            } else {
                upper = mid;
            }
...
public:
    explicit Tree2D(int side) {
        vertice.push_back(Vertex(0, side, 0, side));
    }
    ...
    void add(int coord_x, int coord_y, int delta) {
        ...
        while (!flag_leaf) {
            ...
            if (current_x_right == current_x_left + 1 &&
                ...
                if (next_vertex == -1) {
                    vertice[current_vertex].ptrs[x_move][y_move] = vertice.size();
                    current_vertex = vertice.size();
                    Rectangle sub_rect = Rectangle(current_x_left,
                                                   ...
                                                   x_move,
                                                   y_move);
                    vertice.push_back(Vertex(sub_rect.left_x,
                                             sub_rect.right_x,
                                             sub_rect.left_y,
                                             sub_rect.right_y));
                } else {
    ...
    int sum(int vertex, int left_x, int right_x, int left_y, int right_y) {
        ...
        /*
        if (right_x <= current_x_left || left_x >= current_x_right) {
            return 0;
        }

        if (right_y <= current_y_left || left_y >= current_y_right) {
            return 0;
        }
        */

        // std::cerr << current_x_left << " "
        // << current_x_right << " "
        // << current_y_left << " "
        // << current_y_right << "\n";
        // std::cerr << "came: " << left_x
        // << " " << right_x << " "
        // << left_y << " " << right_y << "\n";
...
std::vector<int> solve(const std::vector<Request>& requests) {
    ...
    /*
    for (const Request& request : requests) {
        std::cerr << maper.lefts_map.getIndex(request.left) << " ";
    }
    std::cerr << "\n";
    for (const Request& request : requests) {
        std::cerr << maper.rights_map.getIndex(request.right) << " ";
    }
    std::cerr << "\n";
    */
