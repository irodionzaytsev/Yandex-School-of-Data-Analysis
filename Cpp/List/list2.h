...
public:
    ...
    void PushBack(const T& elem) = delete;
    void PushBack(T&& elem);
    void PushFront(const T& elem) = delete;
...
template <typename T>
void List<T>::PopBack() {
    Node<T>* tmp = end_->getLeft();
    if (tmp == nullptr) {
        return;
    }
    end_->setLeftPtr(tmp->getLeft());
    if (end_->getLeft() != nullptr) {
        end_->getLeft()->setRightPtr(end_);
    }
    if (begin_ == tmp) {
        begin_ = end_;
    }
